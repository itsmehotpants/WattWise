...
public class AlertServiceRoutes {
    ...
    public RouterFunction<ServerResponse> alertFallbackRoute() {
        return route("alert-fallback")
                .route(RequestPredicates.path("/fallback/alert"),
                        request -> ServerResponse.status(HttpStatus.SERVICE_UNAVAILABLE)
                                .body("Alert service is currently unavailable. Please try again later."))
                .build();
    }
    ...
    public RouterFunction<ServerResponse> alertServiceApiDocs() {
        return GatewayRouterFunctions.route("alert-service-api-docs")
                .route(RequestPredicates.path("/docs/alert-service/v3/api-docs"),
                        http())
                .before(uri("http://localhost:8084"))
