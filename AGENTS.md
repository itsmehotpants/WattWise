...
- Stack: **Spring Boot 4** + **Java 21** for most services; **`insight-service`** uses **Spring Boot 3.5** with **Spring AI** (Ollama). **Spring Cloud 2025.1.2** on the gateway (Gateway Server WebMVC, Resilience4j circuit breakers). No Spring Cloud Config Server in this repo—config is per-service `application.properties`.
...
- MySQL: DB name `energytracker`, init in `docker/mysql/init.sql`; JDBC URLs appear in services' `src/main/resources/application.properties`. Keycloak uses a separate MySQL instance in Compose (`keycloak-db`).

## API Gateway (routing & security)
- Gateway port **8080**; routes under `/api/v1/...` proxy to localhost backends with **circuit breakers** (see `api-gateway/src/main/java/com/leetjourney/api_gateway/route/*.java`).
...
- Service ports (defaults in `application.properties` in this workspace):
  - `api-gateway` **8080**, `device-service` **8081**, `ingestion-service` **8082**, `usage-service` **8083**, `alert-service` **8084**, `insight-service` **8085**, `user-service` **8086**
...
- Through the **gateway** (requires JWT): `http://localhost:8080/api/v1/ingestion` with `Authorization: Bearer <token>`.
